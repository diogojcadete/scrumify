...
import { useProjectMutations } from "./project/projectHooks";
import { 
  fetchProjects, 
  fetchSprints, 
  fetchColumns, 
...
interface ProjectContextType {
  ...
  inviteCollaborator: (data: CollaboratorFormData) => Promise<void>;
...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const {
    ...
    queryFn: () => selectedProject ? fetchCollaborators(selectedProject.id) : [],
  ...
  const isLoading = isProjectsLoading || isSprintsLoading || isColumnsLoading || isTasksLoading || isBacklogItemsLoading || isCollaboratorsLoading || isInvitationsLoading;
  ...
  const inviteCollaborator = async (data: CollaboratorFormData) => {
    if (!selectedProject) {
      toast({
        title: "Error",
        description: "No project selected.",
  ...
  const updateInvitationStatus = async (id: string, status: 'accepted' | 'rejected') => {
    try {
      await updateCollaboratorStatusMutation.mutateAsync({
        id,
        status
      });
      ...
      toast({
        ...
        description: status === 'accepted' 
          ? "You have been added as a collaborator to the project." 
